...
- 免费音频：默认 VAD（可调阈值）、可选「更激进切分」「轻度降噪」；Whisper「灵敏检出」使用 **WhisperSeg**（`whisperseg-asmr`）外部 VAD → `clip_timestamps`，绕过内置 Silero；灵敏预设阈值约 `0.18`、最短人声 `60ms`、最小静音 `140ms`、填充 `350ms`、更长片段合并；长片（≥10 分钟）按 ≤7 分钟定长窗口切分（瞬时，不再全片 ffmpeg 静音扫描），WhisperSeg 只解码一次再内存切片；ASR 后轻量剥离括号音效/SDH 标记，并压缩极端叠字。灵敏检出路径不宜叠轻度降噪或 Demucs 影视增强。可选 ASR `whisper-ja-1.5b`（日语微调）、`anime-whisper`（动画/Galgame；CT2 分窗）、`kotoba-whisper-v2.0-faster`（kotoba 日语蒸馏；长片自动 ≤180s 分窗以防 CT2 `0xC0000005` 崩溃）、**`reazonspeech-k2`**（Reazon Zipformer/sherpa-onnx，自带 subword 时间戳，≤25s 分窗）、**Parakeet**（NeMo，按需 `asr-parakeet`）：`parakeet-tdt-ctc-0.6b-ja`（日语）、`parakeet-tdt-0.6b-v2`（英语+时间戳）、`parakeet-tdt-0.6b-v3`（25 欧语；不含中日韩）。**`cohere-transcribe-03-2026`**（Cohere 2B Conformer，按需 `asr-cohere`；14 语含英日；无原生时间戳，≤30s 短窗 + 句读比例切条；`auto` 语种默认 `en`）。**`av_soft` 默认 `whisper-ja-1.5b`**（可选 `reazonspeech-k2` / `qwen3-asr-0.6b` / `qwen3-asr-1.7b-ja-anime-galgame` / `qwen3-asr-1.7b-ja` / Parakeet JA 等）。**FireRedVAD（随包装）：** 内置 `firered-vad`（约 2MB，无需下载）；`timingAlignModel=firered` 或 VAD 选 `firered-vad` 时，anime-whisper / Qwen 短帧时间轴改用 FireRed（默认仍 TEN）。**Qwen3-ASR 默认 TEN/VAD 帧时间戳**（不加载 ForcedAligner；`timingAlignModel=qwen3-forced-aligner-0.6b` 时启用词级对齐）。**anime-whisper 时间轴默认走 TEN VAD 短帧路径**：静音间隙 0.5s、单帧 ≤5s → 逐帧 ASR，帧边界即字幕时间；帧内按句读/字数比例切条。`timingAlignModel`：`ten`/`auto`/省略 = 该路径；`rate`/`off` = 仅 ≤180s 定长窗 + CPS 修补；`whisper-ja-1.5b` / `wav2vec2` 等 = 二次对时。WhisperSeg 不用于 anime 的 clip 门控（易丢对白）。WhisperSeg GPU 需安装 `onnxruntime-gpu`（与 CPU 包互斥）；ffmpeg 解码仍为 CPU
...
**本地 LLM 显存回收**：软件内模型任务结束后仍可空闲约 5 分钟再停 llama-server（避免连点冷启动）。另：关闭**最后一个**字幕编辑器窗口时会主动卸载空闲 llama（若计算锁仍被首页引擎/TWAI 占用则跳过）；首页引擎批处理与区间重转写在拉起 ASR 前也会先停 llama，腾出显存给识别（批次内 Sakura / 智能翻译需要时再启动）。
