...
(function (global, factory) {
    ...
    const SYSTEM_PROMPT = [
        '你是一个日译中字幕翻译模型，译文需通顺自然，联系上下文正确使用人称代词。',
    ...
    function buildChatMessages(chunkCuesList, options = {}) {
        ...
        const lineRule = nsfw
            ? `共 ${list.length} 行。只输出译文，每行对应原文一行；不要编号、不要解释、不要空行；碎句与拟声勿删并勿并行走。对白有完整语义时禁止只输出语气词（嗯/啊/请/好的等），须译出完整意思。`
            : `共 ${list.length} 行。只输出译文，每行对应原文一行，不要编号、不要解释、不要空行、不要额外内容。对白有完整语义时禁止只输出语气词（嗯/啊/请等）。`;
        let user;
        if (glossary) {
