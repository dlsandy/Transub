...
| 拖入大批文件   | 常需等探测再配参      | **文件名快感优先**；强 AV 番号**立即采纳**，深入感知按需「重感」                 |
...
更细的条目见 [CHANGELOG.md](CHANGELOG.md) · `3.0.0`。
...
| **质量**   | 一般，够用赶工                        | 中上，口语/软声更好                                     | 最高，专名与前后文更稳                                         |
...
- **按项自动感知**：类型 / 语种 / ASR·MT / 音频策略；强番号可秒级采纳  
...
2. 推荐 `Transub-3.0.0-win.zip`，解压后运行 `Transub.exe`
  （或 `Transub-Setup-3.0.0.exe`）  
